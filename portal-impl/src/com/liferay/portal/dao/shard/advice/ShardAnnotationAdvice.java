...
public class ShardAnnotationAdvice
	...
	public Object before(MethodInvocation methodInvocation) throws Throwable {
		...
		SelectionLogic logic = annotation.logic();

		if (annotation.isClassAnnotation()) {
			if (logic.equals(SelectionLogic.THREADLOCAL)) {
				_setShardFromThreadLocal(methodInvocation);
			}
			else if (logic.equals(SelectionLogic.PARAMETER)) {
				_setShardByParameter(methodInvocation);
			}
		}
		else {
			if (logic.equals(SelectionLogic.PARAMETER)) {
				_setShardByParameter(methodInvocation);
			}
		}
	...
	private void _setShardByParameter(MethodInvocation methodInvocation)
		...
		companyIdParam = _getCompanyIdParam(arguments, parameterAnnotations);
			...
			if (_log.isDebugEnabled()) {
				_log.debug(
					"No parameter annotation found for method: " +
						method.getName() + " try to get companyId from first" +
