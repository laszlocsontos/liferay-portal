...
public class LayoutListerTest extends PowerMockito {
	...
	public static void setUpClass() throws Exception {
		...
		when(
			PropsUtil.getArray(Matchers.anyString())).thenReturn(new String[0]);
		...
		when(
			LayoutLocalServiceUtil.getLayouts(
				Matchers.anyLong(),
				Matchers.anyBoolean())).thenReturn(_layouts);

		mockStatic(LocalizationUtil.class);

		when(
			LocalizationUtil.getLocalization()).thenReturn(
	...
	public void testGetLayoutView() throws PortalException, SystemException {
		...
		String[] actualLayoutViewArray = layoutView.getList().toArray(
			new String[0]);
